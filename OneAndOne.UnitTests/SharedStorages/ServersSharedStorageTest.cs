...
namespace OneAndOne.UnitTests.SharedStorages
{
    ...
    public class ServersSharedStorageTest
    {
        ...
        public void CreateSharedStoragesServers()
        {
            ...
            if (sharedStorages.Count == 0)
                return;
            var sharedStorage = sharedStorages[random.Next(sharedStorages.Count - 1)];

            if (servers.Count > 0)
            {
                var currentServer = servers[random.Next(0, servers.Count - 1)];
                ...
                var serverResult = client.SharedStorages.ShowSharedStoragesServer(sharedStorage.Id, currentServer.Id);
